...
Recolore o poster-âncora de p5_02:
  ...
  * gota central → verde, colada de volta SEM halo baked (o glow vira overlay
    procedural no rig)
  * água do tanque, gota da torneira e copinho → intocados (ciano)
  * grava v7/outline_ref.png = outline ESCURO NÍTIDO da gota no poster
    original (barreira anti-vazamento do flood do rig)
...
def main():
    ...
    # 2) gota central: outline nítido de referência (salvo p/ o rig)
    R, G, B = (im[..., i].astype(int) for i in range(3))
    dark = (R < 95) & (G < 95) & (B < 95)
    bb = np.zeros_like(dark)
    bb[200:271, 296:363] = True
    v7dir = os.path.join(args.root, args.mission, 'v7')
    os.makedirs(v7dir, exist_ok=True)
    Image.fromarray(((dark & bb) * 255).astype(np.uint8)).save(
        os.path.join(v7dir, 'outline_ref.png'))

    # interior nítido (poster ainda com outline fechado) + anel
    sil = rigmod.drop_silhouette(im.shape[0], im.shape[1])
    interior = rigmod.drop_interior(im, (296, 200, 362, 270), (232, 329), sil)
    dm = rigmod.ring_of(interior) | interior
    ys, xs = np.nonzero(dm)
    ...
    # some o halo ciano e cole a gota verde
    im = rigmod.inpaint_hole(im, 296, 200, 362, 270)
    am = Image.fromarray((sm * 255).astype(np.uint8)).filter(
        ImageFilter.GaussianBlur(.6))
    base = Image.fromarray(im)
    base.paste(Image.fromarray(sprite), (x0, y0), am)
    im = np.asarray(base)
